/**
   Goal is to emulate MiroWindowsManager from hammerspoon
     ...
     - We'll see about keeping Win hotkey

   FancyZones can let you do 3 columns, use
     # <> to move between the 3 zones
     # !^ <> to make window take up more zones
     BUT can't easily do 1/2 unless maybe do 6 zones, but that's dumb

   Think my solution will be
     - Use # <> to  toggle between sizes 1/3, 1/2, 2/3, full
   ...
   I prefer that snap to left/right makes the window full height, THEN I will SOMETIMES move the window
   ...
   Hammerspoon code i used on Mac:
     ...
     - it uses Hammerspoon's grid feature, which I don't have, so cannot just port the code to AHK
    ...
    Does anything already exist that I can use/modify?
      - These are all very complicated1
